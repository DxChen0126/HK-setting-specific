...
pcontour <- ggplot(mapping = aes(x, y)) + 
  ...
  annotate("text", x = pointdf$x[4] - 0.5 , y = pointdf$y[4] + 0.025, size = 3.5, label = "Close-social indoor") +
  ...
  annotate("text", x = pointdf$x[8] + 0.05, y = pointdf$y[8] - 0.0175, size = 3.5, label = "Care homes") +
  
  geom_point(data = data.frame(x = pointdf$x[9], y = pointdf$y[9]), size = 1.5) +
  annotate("text", x = pointdf$x[9] + 0.7, y = pointdf$y[9], size = 3.5, label = "Nosocomial") +
