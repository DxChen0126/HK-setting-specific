...
p1 <- ggplot(data = df1) +
  geom_boxplot(aes(fill = GI_trunc, y = maxsize, x = k), position = position_dodge(width = 0.75)) + 
#  geom_point(aes(y = maxsize,  x = k, color = GI_trunc), position = "jitter", alpha = 0.1) +
  scale_fill_brewer(palette = "YlGnBu", name = "Truncated GI length") + 
#  scale_color_brewer(palette = "YlGnBu", name = "Truncated GI length") +
...
ggsave("FigS6_20241212.pdf", width = 12.5, height = 9, units = "in")
